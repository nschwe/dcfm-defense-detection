...
const double UDP_PACKET_INTERVAL = 2.0;
...
// Select the black-hole attacker FROM THE ACTUAL FORWARDING PATH (STATE 25.83).
//
// The fixed-node variant below assumes a centre-pinned node sits on data paths.
// Measured 22/8: it does not — the drop never fires, because one UDP flow over a
// 3-hop route has exactly two forwarders among 50 nodes. This reads the source's
// routing table entry for the victim and turns its NEXT HOP — the first real
// forwarder — into the black-hole.
//
// Selection is by topological role and fixed before the measurement, the same
// basis on which ExecuteIsolationAttackByNeighbor picks the isolation attacker.
// It is never chosen by looking at the result.
// Attacker = first node on the source->victim route that is ALSO an MPR
// selector of somebody (STATE 25.84).
//
// Tan et al. §5.1: "only MPRs are responsible for monitoring and evaluating
// their selectors". A node that is nobody's selector is watched by nobody, so
// FPNT cannot score it however badly it behaves — measured: with the plain
// on-path attacker, ZERO nodes held it as a selector, and the defence never
// reacted. This puts the attacker inside the relation the paper makes someone
// responsible for.
//
// Rejects the run when no such node exists. That rejection rate is a RESULT:
// it measures how much of the network an MPR-based trust defence can cover.
static void ExecuteBlackholeAttackOnPathMpr(NodeContainer* nodes, Ipv4Address source,
                                            Ipv4Address target, uint32_t spoofCount){
	// Every address that some node lists as its MPR selector.
	std::set<Ipv4Address> watched;
	for (uint32_t i = 0; i < nodes->GetN(); ++i){
		Ptr<RoutingProtocol> rp = nodes->Get(i)->GetObject<RoutingProtocol>();
		if (!rp) continue;
		for (const auto &sel : rp->GetMprSelectors()){
			watched.insert(sel.mainAddr);
		}
	}

	// Walk the route source -> victim, hop by hop, and take the first hop that
	// is watched. Walking rather than taking hop 1 only: the first forwarder is
	// often nobody's selector, but a later one on the same path may be.
	Ipv4Address curr = source;
	for (uint32_t guard = 0; guard < nodes->GetN(); ++guard){
		Ptr<RoutingProtocol> rp;
		for (uint32_t i = 0; i < nodes->GetN(); ++i){
			Ptr<Ipv4> ip = nodes->Get(i)->GetObject<Ipv4>();
			if (!ip) continue;
			for (uint32_t k = 1; k < ip->GetNInterfaces(); ++k){
				if (ip->GetAddress(k, 0).GetLocal() == curr){
					rp = nodes->Get(i)->GetObject<RoutingProtocol>();
				}
			}
		}
		if (!rp) break;
		...
		Ipv4Address next;
		bool found = false;
		for (const RoutingTableEntry &e : rp->GetRoutingTableEntries()){
			if (e.destAddr == target){ next = e.nextAddr; found = true; break; }
		}
		if (!found || next == target) break;   // reached the victim: no more forwarders

		if (watched.count(next)){
			for (uint32_t i = 0; i < nodes->GetN(); ++i){
				Ptr<Ipv4> ip = nodes->Get(i)->GetObject<Ipv4>();
				if (!ip) continue;
				bool isIt = false;
				for (uint32_t k = 1; k < ip->GetNInterfaces(); ++k){
					if (ip->GetAddress(k, 0).GetLocal() == next) isIt = true;
				}
				if (!isIt) continue;
				Ptr<RoutingProtocol> arp = nodes->Get(i)->GetObject<RoutingProtocol>();
				arp->SetAttribute("IsMalicious", BooleanValue(true));
				arp->SetAttribute("SpoofedLinksCount", UintegerValue(spoofCount));
				std::cout << "Black-hole attacker (on-path, MPR-selector) = node id: " << i
				          << " addr=" << next << " (spoofedLinks=" << spoofCount << ")"
				          << std::endl;
				return;
			}
		}
	...
	std::cout << "Attacker not available: no on-path node is an MPR selector. "
	          << "*** Terminated *** " << std::endl;
	Simulator::Stop();
}

static void ExecuteBlackholeAttackOnPath(NodeContainer* nodes, Ipv4Address source,
                                         Ipv4Address target, uint32_t spoofCount){
	Ptr<Ipv4> srcIpv4;
	Ptr<RoutingProtocol> srcRp;
	for (uint32_t i = 0; i < nodes->GetN(); ++i){
		Ptr<Ipv4> ip = nodes->Get(i)->GetObject<Ipv4>();
		if (!ip) continue;
		for (uint32_t k = 1; k < ip->GetNInterfaces(); ++k){
			if (ip->GetAddress(k, 0).GetLocal() == source){
				srcIpv4 = ip;
				srcRp = nodes->Get(i)->GetObject<RoutingProtocol>();
			}
		}
	}
	if (!srcRp){
		std::cout << "Attacker not available: UDP source " << source
		          << " not found. *** Terminated *** " << std::endl;
		Simulator::Stop();
		return;
	}

	// The source's entry for the victim; its nextAddr is the first forwarder.
	Ipv4Address nextHop;
	bool found = false;
	for (const RoutingTableEntry &e : srcRp->GetRoutingTableEntries()){
		if (e.destAddr == target){
			nextHop = e.nextAddr;
			found = true;
			break;
		}
	}
	if (!found || nextHop == target){
		// No route, or the victim is a direct neighbour: nothing forwards, so a
		// black-hole cannot act. Same disposition as a missing attacker.
		std::cout << "Attacker not available: no forwarder between " << source
		          << " and " << target << ". *** Terminated *** " << std::endl;
		Simulator::Stop();
		return;
	}
	...
	for (uint32_t i = 0; i < nodes->GetN(); ++i){
		...
		bool isIt = false;
		for (uint32_t k = 1; k < ip->GetNInterfaces(); ++k){
			if (ip->GetAddress(k, 0).GetLocal() == nextHop) isIt = true;
		}
		if (!isIt) continue;
		Ptr<RoutingProtocol> rp = nodes->Get(i)->GetObject<RoutingProtocol>();
		rp->SetAttribute("IsMalicious", BooleanValue(true));
		rp->SetAttribute("SpoofedLinksCount", UintegerValue(spoofCount));
		std::cout << "Black-hole attacker (on-path) = node id: " << i
		          << " addr=" << nextHop << " (spoofedLinks=" << spoofCount << ")"
		          << std::endl;
		return;
	}
	std::cout << "Attacker not available: next hop " << nextHop
	          << " has no node. *** Terminated *** " << std::endl;
	Simulator::Stop();
}
...
// ============================================================================
// FPNT second defence (STATE 25.80.13). Selected with --defence=fpnt; the
// default keeps the campaign's own fictitious-node defence, unchanged.
// ============================================================================
std::string g_defence = "gcop";
// FPNT-DIAG (temporary): flat trust for every node -- see FlatTrustDiagnostic.
bool g_fpntFlatTrust = false;

// Installs one trust engine per node. Called once, after the internet stack is
// up so every node already carries a RoutingProtocol. The engine starts
// DISABLED: the window schedule turns it on and off.
static void InstallFpntDefence (NodeContainer* cont)
{
  for (unsigned int i = 0; i < cont->GetN (); ++i)
    {
      Ptr<RoutingProtocol> rp = cont->Get (i)->GetObject<RoutingProtocol> ();
      if (!rp) continue;
      Ptr<IolsrDefenseFpnt> fpnt = CreateObject<IolsrDefenseFpnt> ();
      fpnt->SetAttribute ("FlatTrustDiagnostic", BooleanValue (g_fpntFlatTrust));
      fpnt->SetEnabled (false);
      rp->SetAttribute ("DefenseStrategy", PointerValue (fpnt));
    }
  std::cout << "FPNT defence installed on " << cont->GetN () << " nodes (disabled)."
            << std::endl;
}

static void SetFpntEnabled (NodeContainer* cont, bool on)
{
  for (unsigned int i = 0; i < cont->GetN (); ++i)
    {
      Ptr<RoutingProtocol> rp = cont->Get (i)->GetObject<RoutingProtocol> ();
      ...
      Ptr<IolsrDefenseFpnt> fpnt = DynamicCast<IolsrDefenseFpnt> (pv.GetObject ());
      // SetEnabled is a symmetric cold start: both directions wipe accumulated
      // state, so no window inherits trust from another.
      if (fpnt) fpnt->SetEnabled (on);
    }
}

static void ActivateFictiveDefence(NodeContainer* cont){
	// Dispatch on the selected defence. The window schedule is untouched: it
	// still calls this at the same four points, so both defences share one
	// timeline and cannot drift apart.
	if (g_defence == "fpnt") { SetFpntEnabled (cont, true); return; }
	for (unsigned int i=0; i < cont->GetN(); ++i){
...
static void DeactivateFictiveDefence(NodeContainer* cont){
    if (g_defence == "fpnt") { SetFpntEnabled (cont, false); return; }
    for (unsigned int i = 0; i < cont->GetN(); ++i){
...
int main (int argc, char *argv[]){
	...
	uint32_t bBlackholeOnPath = 0; //0=fixed node id, 1=first forwarder on the route, 2=first forwarder that is also an MPR selector (STATE 25.84)
	...
	cmd.AddValue("bBlackholeOnPath", "Black-hole attacker selection: 0=fixed nAttackerNode, 1=first forwarder on the source->victim route, 2=first on-path forwarder that is also an MPR selector "
	             "(2 reproduces the relation Tan et al. 5.1 puts a monitor in charge of)", bBlackholeOnPath);
	...
	cmd.AddValue("fpntFlatTrust", "FPNT-DIAG (temporary): give every node the "
	             "same trust, so Algorithm 2 runs on a flat metric",
	             g_fpntFlatTrust);
	cmd.AddValue("defence", "Which defence to evaluate: gcop (default, the "
	             "campaign's fictitious-node defence, byte-identical) or fpnt "
	             "(Tan et al. 2015 trust routing, second-defence arm)", g_defence);
	cmd.Parse (argc, argv);
	...
	if (g_windowSeconds < UDP_START_OFFSET_IN_WINDOW + UDP_PACKET_INTERVAL ||
		...
		std::cout << "*** Invalid --windowSeconds=" << g_windowSeconds
		          << " (need >= 6 and (windowSeconds-4) divisible by 2). Terminated." << std::endl;
		return 1;
	}
	...
	// FPNT second defence: install once the stack is up, so every node has a
	// RoutingProtocol to attach to. Disabled until the window schedule says so.
	if (g_defence == "fpnt") { InstallFpntDefence (&nodes); }
	...
	if (bBlackholeAttack) {
		...
		if (bMobility) {
			normalMobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
	...
	if (!g_scenarioOrder.empty()) {
		...
		for (uint32_t slot = 0; slot < 4; ++slot) {
			...
			if (ph.attackOn) {
				if (bBlackholeAttack)
					if (bBlackholeOnPath == 2)
			Simulator::Schedule(Seconds(stabStart[slot]), &ExecuteBlackholeAttackOnPathMpr, &nodes, Ipv4Address("10.0.0.2"), Ipv4Address("10.0.0.1"), nSpoofedLinks);
		else if (bBlackholeOnPath)
			Simulator::Schedule(Seconds(stabStart[slot]), &ExecuteBlackholeAttackOnPath, &nodes, Ipv4Address("10.0.0.2"), Ipv4Address("10.0.0.1"), nSpoofedLinks);
		else
	...
	if (bBlackholeAttack)
		if (bBlackholeOnPath == 2)
			Simulator::Schedule(Seconds(ATTACK_ONLY_STABILIZATION_START), &ExecuteBlackholeAttackOnPathMpr, &nodes, Ipv4Address("10.0.0.2"), Ipv4Address("10.0.0.1"), nSpoofedLinks);
		else if (bBlackholeOnPath)
			Simulator::Schedule(Seconds(ATTACK_ONLY_STABILIZATION_START), &ExecuteBlackholeAttackOnPath, &nodes, Ipv4Address("10.0.0.2"), Ipv4Address("10.0.0.1"), nSpoofedLinks);
		else
	...
	if (bBlackholeAttack)
		if (bBlackholeOnPath == 2)
			Simulator::Schedule(Seconds(DEFENSE_ATTACK_STABILIZATION_START), &ExecuteBlackholeAttackOnPathMpr, &nodes, Ipv4Address("10.0.0.2"), Ipv4Address("10.0.0.1"), nSpoofedLinks);
		else if (bBlackholeOnPath)
			Simulator::Schedule(Seconds(DEFENSE_ATTACK_STABILIZATION_START), &ExecuteBlackholeAttackOnPath, &nodes, Ipv4Address("10.0.0.2"), Ipv4Address("10.0.0.1"), nSpoofedLinks);
		else
