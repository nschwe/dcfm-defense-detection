...
// FPNT monitoring layer (STATE 25.83.3) needs the wifi/LLC/L3 types that the
// plain routing protocol never touched.
...
#include "ns3/node.h"   // PointerValue/MakePointerAccessor for the DefenseStrategy attribute
...
#include "ns3/ipv4-packet-info-tag.h"   // ns-3.47: recover incoming interface in RecvIOlsr
...
bool RoutingProtocol::HasRouteTo(Ipv4Address dst) const
{
    return m_table.find(dst) != m_table.end();
}
...
bool RoutingProtocol::dcfmReqFake(){
...
bool RoutingProtocol::reachableWithDistanceLimitFromNeighbor(const Ipv4Address &src, const Ipv4Address &dest){
	...
	*/
	...
	for (TopologySet::const_iterator it = m_topologySet.begin(); it != m_topologySet.end(); ++it){
		...
		if (m_state.FindTwoHopNeighborTuple(it->lastAddr, src) != NULL) return true;
	}
...
bool RoutingProtocol::RequireFake(){ //New paper - Colors Algorithm - Algorithm 1 Add-Fictitous-Node-Decision
    ...
    //return dcfmReqFake() || sixNodesArrangedInCircle();
...
void RoutingProtocol::disableIsolationAttack() {
    isolationAttack = false;
    isolationAttackTarget = Ipv4Address("0.0.0.0");
    m_willingness = IOLSR_WILL_DEFAULT;
}
void RoutingProtocol::deactivateFictiveMitigation() {
    activeMitigation = false;
}

void RoutingProtocol::deactivateFictiveDefence() {
    activeDefence = false;
}
...
bool RoutingProtocol::MightBeEvil(Ipv4Address x){
	...
	// ns-3.47: m_tcInterval is already a Time; the 3.19 Seconds(Time) implicit
	// path is gone, so add the interval directly. Same quantity.
	if (listenChoose[x] > Simulator::Now() + m_tcInterval){
		return false;
	}
	// Check TC records
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::iolsr::RoutingProtocol")
    ...
    .AddAttribute ("HnaInterval", "HNA messages emission interval.  Normally it is equal to TcInterval.",
                   ...
                   MakeTimeChecker ())
    // ns-3.47: m_willingness is a uint8_t and IOLSR_WILL_* are plain int macros.
    // 3.47's MakeEnumAccessor is now a template requiring an enum type and will
    // not bind a uint8_t member, so this is expressed as a Uinteger attribute.
    // The underlying type, its default (IOLSR_WILL_DEFAULT = 3) and every code
    // path that reads/writes m_willingness are unchanged; only the attribute
    // system's cosmetic string labels are dropped, and nothing sets Willingness
    // by string.
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   UintegerValue (IOLSR_WILL_DEFAULT),
                   MakeUintegerAccessor (&RoutingProtocol::m_willingness),
                   MakeUintegerChecker<uint8_t> ())
    // ====================================================================
    // SECURITY RESEARCH EXTENSION (ported from the student's olsr module):
    // black-hole attack attributes. DEFAULT-OFF -> a node with these unset is
    // behaviourally identical to the original iolsr node, so the existing
    // defence (activeDefence) and node-isolation attack (isolationAttack) are
    // unaffected.
    // ====================================================================
    .AddAttribute ("IsMalicious",
                   "Enable black-hole attack (drop data + manipulate routing). Default false.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&RoutingProtocol::m_isMalicious),
                   MakeBooleanChecker ())
    .AddAttribute ("DefenseStrategy",
                   "The security defence mechanism to use. Null (the default) "
                   "leaves the protocol byte-identical to the campaign build.",
                   PointerValue (),
                   MakePointerAccessor (&RoutingProtocol::m_defenseStrategy),
                   MakePointerChecker<IolsrDefenseStrategy> ())
    .AddAttribute ("SpoofedLinksCount",
                   "Number of real distant nodes falsely advertised as 1-hop (link spoofing). Default 0.",
                   UintegerValue (0),
    ...
    .AddTraceSource ("Rx", "Receive IOLSR packet.",
                     ...
                     "ns3::iolsr::RoutingProtocol::PacketTxRxTracedCallback")
    ...
    .AddTraceSource ("RoutingTableChanged", "The IOLSR routing table has changed.",
                     ...
                     "ns3::iolsr::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
    ...
    m_queuedMessagesTimer (Timer::CANCEL_ON_DESTROY)
{
  ...
  m_hnaRoutingTable = CreateObject<Ipv4StaticRouting> ();  // ns-3.47: Object subclass
  ...
  listenDefence = false;
  // SECURITY RESEARCH EXTENSION (ported black-hole attack): default-off.
  m_isMalicious = false;
...
{
  NS_ASSERT (ipv4);              // ns-3.47: Ptr no longer compares to int 0
  NS_ASSERT (!m_ipv4);
...
{
  // FPNT second defence: release before the protocol's own teardown.
  ...
  if (m_recvSocket)          // ns-3.47: close the shared receive socket too
    {
      m_recvSocket->Close ();
      m_recvSocket = 0;
    }
...
{
  // FPNT second defence: hand the engine its protocol and start its clock.
  // Guarded, so a build with no defence installed is unchanged.
  ...
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      ...
      // ns-3.47 socket model (matches stock olsr 3.47). One receive socket bound
      // to GetAny() receives limited/subnet broadcasts; per-interface send sockets
      // are bound to the interface address and used with SendTo() to the broadcast.
      // The 3.19 single-socket-per-interface-bound-to-unicast model does not
      // receive broadcast on 3.47, which silenced HELLO/TC exchange entirely.
      if (!m_recvSocket)
        {
          m_recvSocket = Socket::CreateSocket (GetObject<Node> (),
                                               UdpSocketFactory::GetTypeId ());
          m_recvSocket->SetAllowBroadcast (true);
          InetSocketAddress inetAddr (Ipv4Address::GetAny (), IOLSR_PORT_NUMBER);
          m_recvSocket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvIOlsr, this));
          if (m_recvSocket->Bind (inetAddr))
            {
              NS_FATAL_ERROR ("Failed to bind() IOLSR receive socket");
            }
          m_recvSocket->SetRecvPktInfo (true);   // so RecvIOlsr can learn the incoming interface
          m_recvSocket->ShutdownSend ();
        }

      // Create a send socket for this interface (send-only)
      ...
      if (socket->Bind (inetAddr))
        {
          NS_FATAL_ERROR ("Failed to bind() IOLSR send socket");
        }
  ...
  // ==========================================================================
  // FPNT monitoring layer (STATE 25.83.3). Called ONCE, here and not earlier:
  // Ipv4L3Protocol only becomes retrievable after the interfaces and sockets
  // above exist, and its "Tx" trace is what feeds OnDataPacketForwarded. Called
  // from DoInitialize instead, the `if (l3)` guard inside silently skipped that
  // connection and every honest next hop was condemned (audit §4).
  //
  // Outside the defence check on purpose: the hooks themselves return
  // immediately when no defence is installed.
  // ==========================================================================
  if (!m_monitorSetupDone)
    {
      SetupPromiscuousMonitor (); // after the socket loop
      m_monitorSetupDone = true;
    }

  // ==========================================================================
  // FPNT: Setup AFTER m_mainAddress exists (STATE 25.84).
  //
  // ⛔ Do NOT move this back into DoInitialize. m_mainAddress is assigned in
  // the interface loop above; calling Setup earlier hands the trust engine an
  // empty address, and a node that does not know its own identity cannot
  // attribute observations to neighbours — every node then converges on the
  // same default trust and attacker and honest node score alike.
  //
  // The periodic timer starts here too: ticking an un-Set-up engine would
  // reason over an empty identity.
  // ==========================================================================
  if (m_defenseStrategy && !m_defenseSetupDone)
    {
...
RoutingProtocol::RecvIOlsr (Ptr<Socket> socket)
{
  ...
  // ns-3.47: the single receive socket is bound to GetAny(), so the incoming
  // interface is recovered from the Ipv4PacketInfoTag (SetRecvPktInfo(true) in
  // DoInitialize) rather than from a per-socket address map.
  Ipv4PacketInfoTag interfaceInfo;
  if (!receivedPacket->RemovePacketTag (interfaceInfo))
    {
      NS_ABORT_MSG ("No incoming interface on IOLSR message, aborting.");
    }
  uint32_t incomingIf = interfaceInfo.GetRecvIf ();
  Ptr<NetDevice> dev = GetObject<Node> ()->GetDevice (incomingIf);
  uint32_t recvInterfaceIndex = m_ipv4->GetInterfaceForDevice (dev);

  InetSocketAddress inetSourceAddr = InetSocketAddress::ConvertFrom (sourceAddress);
  Ipv4Address senderIfaceAddr = inetSourceAddr.GetIpv4 ();
  // Ignore our own broadcasts, which the GetAny() socket would otherwise loop back.
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            ...
            case iolsr::MessageHeader::HELLO_MESSAGE:
              ...
              // FPNT evidence hook: the engine sees the HELLO before the
              // protocol acts on it. The Packet parameter is unnamed in
              // FPNT's signature and never read, so passing a null Ptr is
              // safe and avoids reconstructing one iolsr does not keep here.
              if (m_defenseStrategy)
                {
                  m_defenseStrategy->OnRecvHello (senderIfaceAddr,
                                                  Ptr<const Packet> (),
                                                  messageHeader,
                                                  messageHeader.GetHello ());
                }
            ...
            case iolsr::MessageHeader::TC_MESSAGE:
              ...
              // FPNT evidence hook: TC receipt (see the HELLO note above).
              if (m_defenseStrategy)
                {
                  m_defenseStrategy->OnRecvTc (senderIfaceAddr,
                                               Ptr<const Packet> (),
                                               messageHeader,
                                               messageHeader.GetTc ());
                }
...
{
  // ==========================================================================
  // FPNT: trust routing replaces the whole computation (paper Algorithm 2).
  //
  // ⛔ Do NOT add IsMalicious() filters below for FPNT. FPNT_AUDIT.md 2.3: the
  // paper argues explicitly against blacklisting inside the routing protocol —
  // low-trust nodes are ROUTED AROUND, never isolated. The plain path below,
  // with whatever in-line filters it carries, is for blacklist-style defences
  // or for no defence at all. FPNT leaves before reaching it.
  // ==========================================================================
  if (m_defenseStrategy && m_defenseStrategy->IsTrustRoutingEnabled ())
    {
      RunTrustDijkstra ();
      return;
    }
                            ...
                            const Ipv4Address &senderIface)
{
  ...
  // ==========================================================================
  // FPNT-OLSR: extract piggybacked recommendations (paper Section 5.2).
  //
  // Placed after the symmetric-neighbor and ANSN-freshness checks above, so a
  // stale or duplicate TC that IOLSR itself discards cannot feed the
  // slander-filter aggregation either.
  //
  // The sender passed on is the ORIGINATOR, not the forwarding neighbour: the
  // evaluation was made by the originator, and keying the recommendation map
  // by the forwarder would collapse every originator relayed through the same
  // neighbour into one entry, leaving Eqs. (1)-(3) with a single input and
  // therefore no filtering at all.
  // ==========================================================================
  if (m_defenseStrategy && !tc.evaluationVectors.empty ())
    {
      m_defenseStrategy->OnRecvEvaluationVectors (msg.GetOriginatorAddress (),
                                                  tc.neighborAddresses,
                                                  tc.evaluationVectors);
    }
  ...
  //outputLog.str("");
  outputLog << "This node received TC message from " << msg.GetOriginatorAddress () << " with these nodes inside: ";
  ...
  for (std::vector<Ipv4Address>::const_iterator i = tc.neighborAddresses.begin ();
       i != tc.neighborAddresses.end (); i++)
    {
      ...
      // *** Mitigation: (for tc-hello cross-check), populate the nodesInTcMsg2hops map declared in .h file.
      // *** Should be checked if we need to move the code for efficiency, maybe to the following lines later
      ...
      //       if (two->neighborMainAddr == senderIface){ // senderNode.neighborMainAddr
...
// ============================================================================
// BLACK-HOLE ATTACK helper (ported from student DCFM olsr-routing-protocol.cc).
// Builds a list of REAL, distant node addresses to spoof as direct neighbors.
// Sources (all passively learned, guaranteed to exist and be reachable):
//   1) the 2-hop neighbor set (from neighbors' HELLOs),
//   2) the topology set (from flooded TC messages).
// Own addresses and real 1-hop neighbors are excluded so the lie never
// contradicts the truth (which would itself be a detection signal). The list is
// deterministic so HELLO and TC reference the same spoofed addresses in a given
// cycle. Returns empty when maxCount == 0, so a benign node (the default) is
// unaffected; every caller is additionally gated by m_isMalicious.
// ============================================================================
...
RoutingProtocol::BuildSpoofTargets (uint32_t maxCount) const
{
  ...
  // Exclusion set: own addresses + real 1-hop neighbors/links.
  ...
  const NeighborSet &neighbors = m_state.GetNeighbors ();
  for (NeighborSet::const_iterator nbr = neighbors.begin (); nbr != neighbors.end (); nbr++)
    {
  ...
  // Deduplicate across both sources while preserving deterministic order.
  std::set<Ipv4Address> seen;

  // Source 1: 2-hop neighbors (real nodes our 1-hop neighbors advertised).
  const TwoHopNeighborSet &twoHopSet = m_state.GetTwoHopNeighbors ();
  for (TwoHopNeighborSet::const_iterator twoHop = twoHopSet.begin ();
       twoHop != twoHopSet.end (); twoHop++)
    {
  ...
  // Source 2: topology set (real main addresses harvested from flooded TCs).
  const TopologySet &topology = m_state.GetTopologySet ();
  for (TopologySet::const_iterator topo = topology.begin (); topo != topology.end (); topo++)
    {
...
RoutingProtocol::SendHello ()
{
  ...
  // ============================================================================
  // BLACK-HOLE ATTACK (ported from student DCFM): real-target link spoofing in
  // HELLO. Gated by m_isMalicious (default false). Falsely advertise real,
  // distant nodes (learned passively) as direct symmetric 1-hop neighbors,
  // poisoning the receiver's 2-hop set and MPR computation. Paired with the TC
  // injection in SendTc() for network-wide reach. Additive: benign nodes are
  // unaffected. Mirrors student olsr-routing-protocol.cc SendHello().
  // ============================================================================
  if (m_isMalicious && m_spoofedLinksCount > 0)
    {
      ...
      if (!spoofTargets.empty ())
        {
          iolsr::MessageHeader::Hello::LinkMessage spoofedLinkMessage;
          spoofedLinkMessage.linkCode = (IOLSR_SYM_LINK & 0x03) | ((IOLSR_SYM_NEIGH << 2) & 0x0f);
          for (std::vector<Ipv4Address>::const_iterator addr = spoofTargets.begin ();
               addr != spoofTargets.end (); addr++)
            {
              spoofedLinkMessage.neighborInterfaceAddresses.push_back (*addr);
              NS_LOG_WARN ("MALICIOUS HELLO from " << m_mainAddress
                           << ": falsely advertising real distant node " << *addr
                           << " as a direct symmetric neighbor.");
            }
          linkMessages.push_back (spoofedLinkMessage);
        }
    }
...
RoutingProtocol::SendTc ()
{
  ...
  // ============================================================================
  // BLACK-HOLE ATTACK (ported from student DCFM): ANSN poisoning. Gated by
  // m_isMalicious (default false). Bump the Advertised Neighbor Sequence Number
  // by a large offset so this node's (spoofed) topology update is treated as
  // "newer" than legitimate updates network-wide. Mirrors student
  // olsr-routing-protocol.cc SendTc() (offset 200, modulo IOLSR_MAX_SEQ_NUM).
  // ============================================================================
  if (m_isMalicious)
    {
      m_ansn = (m_ansn + 200) % IOLSR_MAX_SEQ_NUM;
    }
  ...
  if ((isolationAttack) && !((activeDefence) && (RequireFake()))){
	Ipv4Address imaginaryAddress = getFakeAddress();
  ...
  // ============================================================================
  // BLACK-HOLE ATTACK (ported from student DCFM): topology-level link spoofing
  // in TC. Gated by m_isMalicious (default false). HELLOs travel only 1 hop;
  // embedding the same spoofed real-targets in flooded TC poisons the
  // network-wide topology set, so Dijkstra resolves routes to those targets
  // through this attacker. Same target list as the HELLO injection above so the
  // two layers stay consistent. Mirrors student olsr-routing-protocol.cc SendTc().
  // ============================================================================
  if (m_isMalicious && m_spoofedLinksCount > 0)
    {
      std::vector<Ipv4Address> spoofTargets = BuildSpoofTargets (m_spoofedLinksCount);
      for (std::vector<Ipv4Address>::const_iterator addr = spoofTargets.begin ();
           addr != spoofTargets.end (); addr++)
        {
          if (std::find (tc.neighborAddresses.begin (), tc.neighborAddresses.end (), *addr)
  ...
  // ==========================================================================
  // FPNT-OLSR: piggyback this node's evaluation vectors (paper Section 5.2).
  //
  // A TC already advertises exactly the MPR selector set, which is exactly the
  // set of nodes this node is responsible for evaluating, so one vector per
  // advertised address needs no extra control message -- the paper's stated
  // reason for choosing TC as the carrier. Placed last, so the vector list
  // matches the address list even after the black-hole spoof injection above;
  // CarriesEvaluationVectors() enforces that correspondence on the wire.
  // ==========================================================================
  if (m_defenseStrategy)
    {
      tc.evaluationVectors = m_defenseStrategy->GetEvaluationVectors (tc.neighborAddresses);
    }
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
  ...
  if (m_routingTableAssociation)   // ns-3.47: Ptr no longer compares to int 0
    {
                                   ...
                                   const LocalDeliverCallback &lcb, const ErrorCallback &ecb)
{
  ...
  // SECURITY RESEARCH EXTENSION (ported black-hole): drop forwarded data packets.
  // Gated by m_isMalicious (default false) -> benign nodes forward as before.
  if (m_isMalicious)
    {
  ...
  if (Lookup (header.GetDestination (), entry1))
    { 
      ...
      // FPNT evidence hook: the single point where iolsr hands a data
      // packet to a next hop. entry2.nextAddr is the neighbour being trusted,
      // which is precisely what the packet-forwarding-rate factor scores.
      if (m_defenseStrategy)
        {
          m_defenseStrategy->OnDataPacketForwarded (p, entry2.nextAddr,
...
{
  // 't' in Tan et al. Section 5.1: one trust-reasoning period. The engine is
  // deliberately passive about scheduling, so the protocol drives it.
  if (!m_defenseStrategy)
    {
      return;
    }
...
// ============================================================================
// FPNT-OLSR Algorithm 2 — trust based routing (Tan et al. 2015).
//
// Ported verbatim from the audited fpnt347 implementation. Definition 9: the
// path trust is the MINIMUM trust over the INTERMEDIATE nodes; source and
// destination are excluded. The greedy "settle the highest path-trust
// candidate" loop is the bottleneck-Dijkstra Section 4.2 proves correct, with
// hop count as the tie-break (Step 2).
//
// ⛔ This REPLACES RoutingTableComputation while trust routing is enabled.
// Low-trust nodes are routed AROUND, never blacklisted — the paper argues
// explicitly against isolation inside the routing protocol (audit §2.3).
//
// ⛔ The comparator's exact double comparison is deliberate; see the note on
// PathState::operator< below.
// ============================================================================
void
RoutingProtocol::RunTrustDijkstra()
{
    NS_LOG_FUNCTION(this);
    Clear();

    // 1. Build an undirected, deduplicated graph from the OLSR topology state.
    //    Every OLSR relationship we record (symmetric neighbor, 2-hop
    //    neighbor, TC-advertised link) stands for a bidirectional radio link;
    //    storing them directionally would hide otherwise reachable nodes from
    //    the traversal whenever the recorded direction is the wrong way round.
    std::map<Ipv4Address, std::set<Ipv4Address>> graph;
    std::set<Ipv4Address> allNodes;

    auto addEdge = [&](const Ipv4Address& a, const Ipv4Address& b) {
        if (a == b)
        {
    ...
    allNodes.insert(m_mainAddress);

    for (const auto& nb : m_state.GetNeighbors())
    {
        if (nb.status == NeighborTuple::STATUS_SYM)
        {
            addEdge(m_mainAddress, nb.neighborMainAddr);
        }
    }

    for (const auto& nb2 : m_state.GetTwoHopNeighbors())
    {
        addEdge(nb2.neighborMainAddr, nb2.twoHopNeighborAddr);
    }

    for (const auto& topo : m_state.GetTopologySet())
    {
        // Links incident on us are already covered by the (authoritative)
        // symmetric neighbor set; a TC claiming such a link is exactly the
        // link-spoofing lie we must not let into the graph.
        if (topo.lastAddr == m_mainAddress || topo.destAddr == m_mainAddress)
        {
            continue;
        }
        addEdge(topo.lastAddr, topo.destAddr);
    }

    // 2. Initialize the search state. TAB_PT is represented implicitly by
    //    settled entries in maxTrust/parent; TAB_CAN is the priority queue.
    std::map<Ipv4Address, double> maxTrust;
    std::map<Ipv4Address, Ipv4Address> parent;
    std::map<Ipv4Address, uint32_t> hops;

    for (const auto& node : allNodes)
    {
        maxTrust[node] = -1.0;
        hops[node] = UINT32_MAX;
    }
    maxTrust[m_mainAddress] = 1.0; // root entry {(V_i, 1, V_i)} of Step 1
    hops[m_mainAddress] = 0;
    ...
    struct PathState
    {
        ...
        // Strict weak ordering on (trust DESC, hops ASC, addr ASC).
        // Exact double comparison is deliberate: trust values come from
        // uint8-quantized fields (ev.trust / 255.0), so the bit patterns are
        // reproducible, and an epsilon-tolerant equivalence would break
        // transitivity and with it the std::set invariants.
    ...
    std::set<PathState> q;
    q.insert({1.0, 0, m_mainAddress});

    const bool hasDefense = (m_defenseStrategy != nullptr);

    // 3. Greedy loop: repeatedly settle the candidate with the highest path
    //    trust (Algorithm 2, Step 3) and relax its neighbors (Step 2).
    while (!q.empty())
    {
        const PathState top = *q.begin();
        q.erase(q.begin());

        const Ipv4Address u = top.addr;
        const double u_trust = top.trust; // path trust from us to u
        const uint32_t u_hops = top.hops;
        ...
        {
            // Definition 9: PT is the minimum trust over the INTERMEDIATE
            // nodes of the path. Relaxing (u, v) folds in the trust of u --
            // the node being traversed. The source contributes a trivial 1.0
            // and v, which may be this path's destination, is excluded.
            double u_node_trust = 1.0;
            if (hasDefense && u != m_mainAddress)
            {
                u_node_trust = m_defenseStrategy->GetNodeTrust(u);
            }

            const double pathTrust = std::min(u_trust, u_node_trust);
            const uint32_t pathHops = u_hops + 1;

            bool update = false;
            if (pathTrust > maxTrust[v])
            {
                update = true;
            }
            else if (pathTrust == maxTrust[v] && pathHops < hops[v])
            {
                update = true; // Step 2 tie-break: equal trust, shorter path
            }
            ...
            if (update)
            {
                if (maxTrust[v] != -1.0)
                {
                    auto it = q.find({maxTrust[v], hops[v], v});
                    if (it != q.end())
                    {
                        q.erase(it);
                    }
                ...
                maxTrust[v] = pathTrust;
                hops[v] = pathHops;
                parent[v] = u;
                q.insert({pathTrust, pathHops, v});
            }
        }
    }

    // 4. Turn the parent tree into routing entries by backtracking to the
    //    first hop out of this node.
    for (const auto& pair : parent)
    {
        const Ipv4Address dest = pair.first;
        if (dest == m_mainAddress)
        {
            continue;
        }

        Ipv4Address curr = dest;
        Ipv4Address nextHop = curr;
        ...
        // AddEntry expects the next hop's INTERFACE address (that is what
        // ARP resolves on the outgoing link), not its main address, which is
        // the convention every other AddEntry call in this file follows.
        for (const auto& link : m_state.GetLinks())
        {
            if (GetMainAddress(link.neighborIfaceAddr) == nextHop &&
                link.symTime >= Simulator::Now())
            {
                const int32_t interfaceIndex =
                    m_ipv4->GetInterfaceForAddress(link.localIfaceAddr);
                if (interfaceIndex != -1)
                {
                    AddEntry(dest,
                             link.neighborIfaceAddr,
    ...
    // 5. MID: every other interface of a node we can already reach shares
    //    that node's route. Stock RoutingTableComputation does this too;
    //    without it, traffic addressed to a secondary interface is dropped.
    for (const auto& midTuple : m_state.GetIfaceAssocSet())
    {
        RoutingTableEntry mainEntry;
        if (!Lookup(midTuple.mainAddr, mainEntry))
        {
            continue;
        }
        RoutingTableEntry unused;
        if (Lookup(midTuple.ifaceAddr, unused))
        {
            continue;
        }
        AddEntry(midTuple.ifaceAddr,
                 mainEntry.nextAddr,
                 mainEntry.interface,
                 mainEntry.distance);
    }

    // 6. HNA: gateway-advertised external networks, routed via the gateway's
    //    (now trust-selected) entry.
    for (uint32_t i = 0; i < m_hnaRoutingTable->GetNRoutes(); i++)
    {
        m_hnaRoutingTable->RemoveRoute(0);
    }

    for (const auto& tuple : m_state.GetAssociationSet())
    {
        bool weAreTheGateway = false;
        for (const auto& local : m_state.GetAssociations())
        {
            if (local.networkAddr == tuple.networkAddr && local.netmask == tuple.netmask)
            {
                weAreTheGateway = true;
                break;
            }
        }
        if (weAreTheGateway)
        ...
        RoutingTableEntry gatewayEntry;
        if (Lookup(tuple.gatewayAddr, gatewayEntry))
        {
            m_hnaRoutingTable->AddNetworkRouteTo(tuple.networkAddr,
                                                 tuple.netmask,
                                                 gatewayEntry.nextAddr,
...
// ============================================================================
// FPNT promiscuous monitoring (STATE 25.83.3).
//
// The trust engine's packet-forwarding-rate factor needs BOTH halves:
//   MonitorIpv4Tx     -- what this node handed to a neighbour
//   MonitorSnifferRx  -- whether that neighbour passed it on (the watchdog)
// Without the second, a forwarding RATE cannot be computed at all.
//
// Ported verbatim from the audited fpnt347 implementation. Every entry point
// returns immediately when no defence is installed.
// ============================================================================
bool
RoutingProtocol::IsOwnMacAddress (Mac48Address mac) const
{
    ...
    for (uint32_t i = 0; i < node->GetNDevices (); ++i)
    {
        ...
        if (wifiDev && Mac48Address::ConvertFrom (wifiDev->GetAddress ()) == mac)
        {
            return true;
        }
    }
...
RoutingProtocol::MonitorSnifferRx (Ptr<const Packet> packet, uint16_t channelFreqMhz, WifiTxVector txVector, MpduInfo aMpdu, SignalNoiseDbm signalNoise, uint16_t staId)
{
    ...
    if (wifiHdr.IsData())
    {
        ...
        // The PHY sniffer also reports our own transmissions. Reporting them
        // would double-count: traffic we send to a neighbor is already
        // accounted for by OnDataPacketForwarded, so a second receiver-side
        // count here would inflate that neighbor's load and its packet
        // forwarding rate denominator.
        if (IsOwnMacAddress (transmitter))
        {
            return;
        }
...
void
RoutingProtocol::MonitorIpv4Tx (Ptr<const Packet> packet, Ptr<Ipv4>, uint32_t)
{
    if (!m_defenseStrategy || !packet)
    {
        return;
    }
    ...
    if (dst.IsBroadcast () || dst.IsMulticast () || dst == Ipv4Address::GetAny ())
    {
        return;   // OLSR control traffic and the like: not a forwarding obligation
    }

    // The next hop is our own routing decision for this datagram; no
    // simulator-global knowledge is involved.
    RoutingTableEntry entry;
    if (!Lookup (dst, entry))
    {
        return;
    ...
    m_defenseStrategy->OnDataPacketForwarded (header, payload, entry.nextAddr, dst);
}
...
RoutingProtocol::SetupPromiscuousMonitor ()
{
    ...
    // Hook 3: outgoing datagrams with their final IPv4 header. See
    // MonitorIpv4Tx for why RouteOutput's header cannot serve this purpose.
    Ptr<Ipv4L3Protocol> l3 = node->GetObject<Ipv4L3Protocol> ();
    if (l3)
    {
        l3->TraceConnectWithoutContext ("Tx", MakeCallback (&RoutingProtocol::MonitorIpv4Tx, this));
    }
}

void
RoutingProtocol::OnLocalRxDrop (Ptr<const Packet> packet,
                                ns3::WifiPhyRxfailureReason reason)
{
  // Counts local PHY drops (collision / noise). Referenced by
  // SetupPromiscuousMonitor; consumed only by the DoS trust factors, which this
  // campaign does not exercise.
  m_localRxDrops++;
}
