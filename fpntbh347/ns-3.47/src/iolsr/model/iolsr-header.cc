...
#define IOLSR_MSG_HEADER_SIZE 12
#define IOLSR_PKT_HEADER_SIZE 4

namespace ns3 {
namespace iolsr {


NS_LOG_COMPONENT_DEFINE ("IOlsrHeader");

/// Scaling factor used in RFC 3626.
#define IOLSR_C 0.0625
...
SecondsToEmf (double seconds)
{
  ...
  for (b = 0; (seconds/IOLSR_C) >= (1 << b); ++b)
    ;
  NS_ASSERT ((seconds/IOLSR_C) < (1 << b));
  b--;
  NS_ASSERT ((seconds/IOLSR_C) >= (1 << b));

  // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
  double tmp = 16*(seconds/(IOLSR_C*(1<<b))-1);
...
///
/// \param iolsr_format number of seconds in mantissa/exponent format.
/// \return the decimal number of seconds.
///
double
EmfToSeconds (uint8_t iolsrFormat)
{
  int a = (iolsrFormat >> 4);
  int b = (iolsrFormat & 0xf);
  // value = C*(1+a/16)*2^b [in seconds]
  return IOLSR_C * (1 + a/16.0) * (1 << b);
}
...
{
  static TypeId tid = TypeId ("ns3::iolsr::PacketHeader")
    .SetParent<Header> ()
...
{
  static TypeId tid = TypeId ("ns3::iolsr::MessageHeader")
    .SetParent<Header> ()
...
{
  uint32_t size = IOLSR_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  size = IOLSR_MSG_HEADER_SIZE;
  switch (m_messageType)
    {
    case MID_MESSAGE:
      size += m_message.mid.Deserialize (i, m_messageSize - IOLSR_MSG_HEADER_SIZE);
      break;
    case HELLO_MESSAGE:
      size += m_message.hello.Deserialize (i, m_messageSize - IOLSR_MSG_HEADER_SIZE);
      break;
    case TC_MESSAGE:
      size += m_message.tc.Deserialize (i, m_messageSize - IOLSR_MSG_HEADER_SIZE);
      break;
    case HNA_MESSAGE:
      size += m_message.hna.Deserialize (i, m_messageSize - IOLSR_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
...
/// A plain IOLSR TC carries 0 there, so the two encodings stay
/// distinguishable and a defence-free run is bit-for-bit unchanged.
...
MessageHeader::Tc::Deserialize (Buffer::Iterator start, uint32_t messageSize)
{
  ...
  const uint16_t reserved = i.ReadNtohU16 (); // Reserved / FPNT marker
  ...
  if (payloadSize % entrySize != 0)
    {
      ...
      NS_LOG_WARN ("Malformed TC message: payload " << payloadSize
                   << " not a multiple of " << entrySize);
      return messageSize;
    }
