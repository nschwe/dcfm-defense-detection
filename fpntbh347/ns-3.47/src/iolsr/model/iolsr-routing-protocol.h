...
#include "ns3/wifi-phy.h"          // WifiPhyRxfailureReason, MpduInfo
#include "ns3/wifi-tx-vector.h"    // WifiTxVector
#include "ns3/mac48-address.h"   // FPNT second defence (STATE 25.80)
...
public:
  ...
  // SECURITY RESEARCH EXTENSION (ported black-hole attack): default-off flags +
  // helper. m_isMalicious=false / m_spoofedLinksCount=0 => identical to original.
  bool m_isMalicious;
...
//   std::map<Ipv4Address, std::set<Ipv4Address> > oneHopNodesWhoAppearedInTCmessageFrom2hop; //(change variable name) key = sender of TC, Value = set of Ipv4Addresses of nodes inside of the TC message.
  ...
  **/
  ...
  void disableIsolationAttack();
  void deactivateFictiveDefence();
  void deactivateFictiveMitigation();
  bool HasRouteTo(Ipv4Address dst) const;
  ...
  // ==========================================================================
  // FPNT adapter (STATE 25.80.3).
  //
  // iolsr holds every quantity the FPNT trust engine needs, under different
  // names. These forwarders expose exactly the spellings the engine calls, so
  // `iolsr-defense-fpnt.{h,cc}` stays byte-identical to the audited fpnt347
  // implementation. Nothing here adds behaviour -- each is one delegation.
  //
  // ⛔ Do not "simplify" by renaming inside FPNT instead. Fidelity to
  //    Tan et al. 2015 rests on that file being an unmodified copy.
  // ==========================================================================
  const NeighborSet& GetNeighbors () { return getNeighborSet (); }
  const TwoHopNeighborSet& GetTwoHopNeighbors () { return getTwoHopNeighborSet (); }
  const MprSet GetMprSet () { return getMprSet (); }
  const MprSelectorSet& GetMprSelectors () const { return m_state.GetMprSelectors (); }
  void RecomputeRoutingTable () { RoutingTableComputation (); }

  /// GetMainAddress already carries the name FPNT calls; it is merely private.
  /// Granted by friendship rather than a public wrapper, so the protocol's
  /// public surface does not grow to accommodate one defence.
  friend class IolsrDefenseFpnt;
...
private:
  ...
  // ==========================================================================
  // FPNT second defence (STATE 25.80.3). Null unless a scenario installs one
  // via the "DefenseStrategy" attribute, so every existing campaign behaves
  // exactly as before: no object, no hooks, no cost.
  // ==========================================================================
  Ptr<IolsrDefenseStrategy> m_defenseStrategy;
  Timer m_defenseTimer;              ///< drives PeriodicCheck ('t' in the paper)
  void HandleDefenseTimer ();        ///< PeriodicCheck() + reschedule
  ...
  /// FPNT Algorithm 2: bottleneck-Dijkstra on node trust.
  /// Replaces RoutingTableComputation while trust routing is on.
  void RunTrustDijkstra ();

  /// FPNT monitoring layer (STATE 25.83.3). Feeds the trust engine's
  /// packet-forwarding-rate factor; no-ops when no defence is installed.
  bool IsOwnMacAddress (Mac48Address mac) const;
  void MonitorSnifferRx (Ptr<const Packet> packet, uint16_t channelFreqMhz,
                         WifiTxVector txVector, MpduInfo aMpdu,
                         SignalNoiseDbm signalNoise, uint16_t staId);
  void MonitorIpv4Tx (Ptr<const Packet> packet, Ptr<Ipv4>, uint32_t);
  void SetupPromiscuousMonitor ();
  void OnLocalRxDrop (Ptr<const Packet> packet, ns3::WifiPhyRxfailureReason reason);
  uint32_t m_localRxDrops {0};
  bool m_monitorSetupDone {false};
  bool m_defenseSetupDone {false};
  ...
  std::map< Ptr<Socket>, Ipv4InterfaceAddress > m_socketAddresses;  // send sockets (per interface)
  // ns-3.47: a single receive socket bound to GetAny() so it receives limited/
  // subnet broadcasts. On 3.47 a socket bound to a unicast interface address does
  // not receive broadcast frames, which silenced OLSR HELLO/TC exchange entirely.
  Ptr<Socket> m_recvSocket;

public:
  // ns-3.47: AddTraceSource now requires a 4th argument naming the callback
  // signature. These typedefs provide those names (same as stock olsr 3.47).
