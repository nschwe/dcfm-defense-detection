...
Output:
  - results/confusion_matrices_universal3/confusion_static_in_domain.csv
  - results/confusion_matrices_universal3/confusion_static_to_mobile.csv
  - results/confusion_matrices_universal3/confusion_mobile_in_domain.csv
  - results/confusion_matrices_universal3/confusion_mobile_to_static.csv
  - results/confusion_matrices_universal3/confusion_matrices_summary.txt
  - results/confusion_matrices_universal3/confusion_matrices.png (4-panel)

Usage:
    python3 confusion_matrices_universal3_v2.py \
        --static-root ../simulations/features_static \
        --mobile-root ../simulations/features_mobile \
        --hp-results-dir ./results/hp_search_extended \
        --out-dir ./results/confusion_matrices_universal3
...
# The pipeline decides the feature space; this script lives in analysis/ and
# must be told where it is. Same convention as r33_sourceonly_k.py.
#   analysis/pipeline                    -> METRICS = 33
#   <arm>/pipeline_17, cleanfeat/pipeline_17 -> METRICS = 17
import pathlib as _pl
_HERE = _pl.Path(__file__).resolve().parent
R45_PIPELINE = os.environ.get(
    "R45_PIPELINE", str(_HERE / "arms_r34_17feat" / "pipeline_17"))
if not os.path.isfile(os.path.join(R45_PIPELINE, "defense_detection_v2.py")):
    raise SystemExit(f"ABORT: no defense_detection_v2.py under {R45_PIPELINE}")
sys.path.insert(0, R45_PIPELINE)
print(f"pipeline  : {R45_PIPELINE}")
...
# Universal-3, the reported 17-observable campaign's set:
#   analysis/arms_r34_17feat/listener/results/
#     feature_importance_sensitivity_v2/universal_set_k3/universal_set.txt
# DataPacketRate and FlowThroughputStd belonged to the superseded Universal-4
...
def evaluate_seed(X_src, y_src, g_src, X_tgt, y_tgt, g_tgt,
                  best_params, seed):
    """Run one seed: train on the source 60% partition, evaluate in-domain on
    the source TEST partition and cross-domain on the full target.

    CORRECTED 30/8 for R#4.5. The previous version evaluated in-domain on the
    FULL source and admitted it in this docstring: the in-domain panel mixed
    train/val/test predictions. A confusion matrix presented as an evaluation
    must not include rows the model trained on, whatever the display
    convention.

    The two panels therefore have different and honest sizes:
      in-domain    = source test partition     (20% of the source)
      cross-domain = full target configuration (never seen in training)
    """
    ...
    X_te_s  = sc.transform(X_src.iloc[te])   # held-out test partition ONLY
    ...
    p_src = clf.predict_proba(X_te_s)[:, 1]
    ...
    return {
        "y_te_true":  y_src.iloc[te].to_numpy(),
...
def aggregate_confusion(results, key_true, key_pred, target_total=None):
    """Aggregate confusion matrices across seeds.

    Summing per-seed matrices would inflate every cell by a factor of
    n_seeds, so the per-seed matrices are averaged. The reported counts are
    therefore the mean matrix, and they represent one panel's own evaluation
    population: `target_total` defaults to the mean matrix's own total, so an
    in-domain panel reports the held-out test partition and a cross-domain
    panel the full target configuration.

    An explicit `target_total` rescales the matrix to that total instead. It
    is not the default and must not be used to give panels a common count:
    rescaling every panel to 40,000 is precisely what made the in-domain
    panels appear to have been evaluated on the whole configuration, which is
    the defect R#4.5 reports and the 30/8 correction below removes.

    Whichever total is used, the cell ratios and the displayed percentages
    are preserved exactly; only the absolute counts are rounded to integers,
    by largest-remainder rounding, so that they sum to the total.
    """
    cms = []
    accs = []
    aucs = []
    ...
    # CORRECTED 30/8 (R#4.5): default to the panel's OWN sample count instead
    # of rescaling every panel to 40,000. Rescaling is what made the in-domain
    # panels appear to have been evaluated on the whole configuration.
    if target_total is None:
        target_total = int(round(cm_mean.sum()))
    s = cm_mean.sum()
    if s <= 0:
    ...
    return cm_norm, np.mean(accs), np.std(accs, ddof=1), np.mean(aucs), np.std(aucs, ddof=1)
...
def save_cm_csv(cm, path, n_seeds, acc_mean, acc_std, auc_mean, auc_std):
    ...
    with open(path, "w") as f:
        ...
        # The mean matrix's own total, i.e. the evaluation population of this
        # panel. NOT a sum over seeds: summing 20 seeds would give 20x this.
        f.write(f"# Samples represented by the mean confusion matrix: {n_total}\n")
...
def plot_confusion_matrices(cm_dict, out_path):
    ...
    fig.suptitle("Confusion Matrices \u2014 Universal 3 Features\n"
...
def main():
    ...
    parser.add_argument("--out-dir",
                         default="./results/confusion_matrices_universal3")
    ...
    print("=" * 70)
    print(f"  Universal-3 features: {UNIVERSAL_4}")
    ...
    with open(out_dir / "confusion_matrices_summary.txt", "w") as f:
        f.write("=" * 70 + "\n")
        f.write("Confusion Matrices \u2014 Universal-3 Feature Set (CatBoost)\n")
        f.write("=" * 70 + "\n\n")
