...
def load_headline_feature_sets(k_sweep_csv, all_features, k_full=17, anchor=None):  # PATCHED COPY: 33 -> 17
    """Load the EXACT feature sets the headline used, so the decomposition runs
    on identical scores.  K=len(anchor) -> the universal/anchor set;
    K=k_full -> the full-space subset recorded in k_sweep_results.csv (for the
    original arm the pivoted data has 34 columns; k_sweep's importance ranking
    drops the metadata column `_measurement_duration`, so K=33 != all 34
    columns).  Feature order is irrelevant to CatBoost.

    17/8 (STATE §25.11 blocker 7): k_full and anchor were hardcoded 33 /
    UNIVERSAL_4, which crashes on any arm whose full space is not 33 (the
    listener's is ~21). Defaults preserve the old behaviour exactly.
    """
    if anchor is None:
        raise ValueError("anchor is required: the built-in UNIVERSAL_4 is the "
                         "submitted four-feature set, not the anchor of the "
                         "revised analysis. Pass --universal-set.")
    if len(anchor) == k_full:
        raise ValueError(f"anchor size ({len(anchor)}) equals --k-full "
                         f"({k_full}); the two sets must differ")
    df = pd.read_csv(k_sweep_csv)
    row = df[(df.K == k_full) & (df.direction == "S_to_M")]
    if row.empty:
        raise ValueError(f"No K={k_full} / S_to_M row in {k_sweep_csv}")
    feats_full = row.iloc[0]["features"].split(";")
    assert len(feats_full) == k_full, \
        f"expected {k_full} features, got {len(feats_full)}"
    missing = [f for f in feats_full if f not in all_features]
    if missing:
        raise ValueError(f"K={k_full} features absent from data columns: {missing}")
    missing_a = [f for f in anchor if f not in all_features]
    if missing_a:
        raise ValueError(f"anchor features absent from data: {missing_a}")
    return {k_full: feats_full, len(anchor): list(anchor)}
...
def run_direction(direction, X_src, y_src, g_src, X_tgt, y_tgt, g_tgt,
    ...
    for seed in seeds:
        ...
        # KS[0] = full space, KS[1] = anchor set. With the defaults (33, 4)
        # every column name below is byte-identical to the pre-17/8 output.
        KF, KA = KS[0], KS[1]
        d_total = acc_src[KA] - acc_src[KF]
        d_ranking = acc_orc[KA] - acc_orc[KF]
        d_threshold = d_total - d_ranking
        assert abs(d_total - (d_ranking + d_threshold)) < 1e-12, "identity broke"

        row.update({
            f"acc_src_K{KF}": acc_src[KF], f"acc_src_K{KA}": acc_src[KA],
            f"acc_orc_K{KF}": acc_orc[KF], f"acc_orc_K{KA}": acc_orc[KA],
            f"acc_orc_intest_K{KF}": acc_orc_intest[KF],
            f"acc_orc_intest_K{KA}": acc_orc_intest[KA],
            "delta_total": d_total, "delta_ranking": d_ranking,
            "delta_threshold": d_threshold,
            f"t_src_K{KF}": t_src_K[KF], f"t_src_K{KA}": t_src_K[KA],
            f"t_orc_K{KF}": t_orc_K[KF], f"t_orc_K{KA}": t_orc_K[KA],
            f"acc_src_fulltarget_K{KF}": acc_src_full[KF],
            f"acc_src_fulltarget_K{KA}": acc_src_full[KA],
        })
        ...
        print(f"  seed {seed}: d_total={d_total:+.4f} d_rank={d_ranking:+.4f} "
              f"d_thr={d_threshold:+.4f} | acc_src(K{KF}/K{KA})={acc_src[KF]:.4f}/"
              f"{acc_src[KA]:.4f} ({time.time()-t0:.0f}s){flag}")
...
def build_summary_and_components(direction, res, out_rows_summary,
    ...
    # 2x2x3 summary table
    regime_cols = {
        "source_thr": (f"acc_src_K{KS[0]}", f"acc_src_K{KS[1]}"),
        "oracle_calib": (f"acc_orc_K{KS[0]}", f"acc_orc_K{KS[1]}"),
        "oracle_intest": (f"acc_orc_intest_K{KS[0]}", f"acc_orc_intest_K{KS[1]}"),
    }
    for regime, (c33, c4) in regime_cols.items():
        for K, col in ((KS[0], c33), (KS[1], c4)):
    ...
    # across K, so run_codes/n_runs taken from the full-space entry).
    paired_seed_data = []
    for si in range(len(bsd[KS[0]])):
        d33, d4 = bsd[KS[0]][si], bsd[KS[1]][si]
...
def main():
    ...
    # 17/8 (STATE §25.11 blocker 7). Defaults reproduce the old behaviour
    # exactly; all three must be set for any arm whose space is not 33/4.
    ap.add_argument("--k-sweep-csv",
                    default="./results/k_sweep_universal4/k_sweep_results.csv",
                    help="k_sweep_results.csv holding the full-space feature "
                         "list (run_arm passes the arm's k_sweep subdir)")
    ap.add_argument("--k-full", type=int, default=17,  # PATCHED COPY: 33 -> 17
                    help="size of the full feature space (33 for the original "
                         "arm; the listener's is smaller)")
    ap.add_argument("--universal-set", required=True,
                    help="anchor set: comma-list or file with one name per "
                         "line. REQUIRED since 16/9: the built-in UNIVERSAL_4 is "
                         "the submitted four-feature set and is never the anchor "
                         "of the revised analysis, so a bare invocation used to "
                         "decompose against the wrong set in silence.")
    args = ap.parse_args()

    anchor = None
    if args.universal_set:
        if os.path.isfile(args.universal_set):
            with open(args.universal_set) as fh:
                anchor = [ln.strip() for ln in fh
                          if ln.strip() and not ln.startswith("#")]
        else:
            anchor = [f.strip() for f in args.universal_set.split(",") if f.strip()]
        if not anchor:
            raise ValueError(f"--universal-set {args.universal_set!r} "
                             f"yielded an empty feature list")
    # KS drives every accumulator and column name: [full space, anchor].
    globals()["KS"] = [args.k_full, len(anchor)]
    ...
    feats = load_headline_feature_sets(args.k_sweep_csv, all_features,
                                       k_full=args.k_full, anchor=anchor)
    print(f"  total columns: {len(all_features)} | "
          f"K={KS[0]} set: {len(feats[KS[0]])} | K={KS[1]} set: {len(feats[KS[1]])}")
