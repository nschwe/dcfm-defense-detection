...
def analyze_config(name, data_root, model_path, out_root="results"):
    print(f"\n{'=' * 60}\n  {name.upper()}\n{'=' * 60}")
    ...
    out_path = f"{out_root}/{name}/variance_decomposition.csv"
...
if __name__ == "__main__":
    # argparse added 17/8 (STATE §22.0 blocker: no argparse, hardcoded paths).
    # Every default equals the old hardcoded value, so a bare invocation is
    # unchanged. --hp-results-dir is accepted (run_arm passes it to this script
    # family) but unused: the trained Stacking pickle already embeds its params.
    import argparse
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--static-root", default="../simulations/features_static")
    ap.add_argument("--mobile-root", default="../simulations/features_mobile")
    ap.add_argument("--results-root", default="results",
                    help="dir holding {static,mobile}/best_model_Stacking_Ensemble.pkl")
    ap.add_argument("--out-dir", default="results",
                    help="output root; CSVs land in <out-dir>/{static,mobile}/")
    ap.add_argument("--hp-results-dir", default=None, help="accepted, unused")
    args = ap.parse_args()

    analyze_config("static",
                   args.static_root,
                   os.path.join(args.results_root, "static",
                                "best_model_Stacking_Ensemble.pkl"),
                   out_root=args.out_dir)
    analyze_config("mobile",
                   args.mobile_root,
                   os.path.join(args.results_root, "mobile",
                                "best_model_Stacking_Ensemble.pkl"),
                   out_root=args.out_dir)