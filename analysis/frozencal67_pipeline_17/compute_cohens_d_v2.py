...
Absolute Cohen's d = |mu1 - mu0| / pooled_std   (the absolute value is deliberate)
...
#
# 16/9: the list above marked the SUBMITTED four-feature set, which the
# revised analysis replaced. The selection that supersedes it is
# feature_importance_sensitivity_v2, whose K = 3 output is the set below;
# the column it writes was named universal_provisional until the same day.
UNIVERSAL_3 = [
    "AverageAdvertisedLinksPerTCMessage",
    "AverageMprCount",
    "TcMessageRate",
...
def cohens_d(x0, x1):
    """Absolute Cohen's d = |mu1 - mu0| / pooled_std"""
    n0, n1 = len(x0), len(x1)
...
def main():
    ...
    for feat in METRICS:
        rows.append({
            ...
            "universal_3": "yes" if feat in UNIVERSAL_3 else "no",
        })
    ...
    print(f"\n{'='*70}")
    print("UNIVERSAL-3, the selected cross-domain subset")
    print(f"{'='*70}")
    print(f"{'Feature':<40} {'Static d':>9} {'Mobile d':>9} {'Delta':>8}")
    print("-"*70)
    df_u7 = df[df["universal_3"] == "yes"].sort_values("static_d", ascending=False)
    ...
    for _, row in df.iterrows():
        u7 = "✓" if row["universal_3"] == "yes" else ""
