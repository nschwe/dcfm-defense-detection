...
class DefenseDetector:
    ...
    def calibrate_models(self, models: Dict[str, Any], X_cal: pd.DataFrame,
        ...
        # 1/9 FROZEN-CALIBRATION FIX. The previous form, CalibratedClassifierCV(
        # model, cv=5), CLONES AND REFITS the classifier on X_cal: the evaluated
        # model was no longer the one fitted on the training partition but five
        # models fitted on 4/5 of the calibration half. cv="prefit" is rejected
        # outright by sklearn 1.9.0 (InvalidParameterError); FrozenEstimator is
        # the supported way to fit only the probability mapping. Verified: one
        # inner estimator, identical to the trained classifier.
        n_frozen = 0
        for name, model in models.items():
            try:
                ...
                inner = calibrated.calibrated_classifiers_
                if len(inner) != 1:
                    raise RuntimeError(
                        f"{name}: expected 1 frozen inner estimator, got {len(inner)}")
                n_frozen += 1
                calibrated_models[name] = calibrated
            except Exception as exc:
                # Loud, not silent: the old bare `except` is exactly how the
                # refit went unnoticed.
                self.log(f"  ⚠️ Calibration of {name} failed ({exc}); "
                         f"keeping the UNCALIBRATED model")
                calibrated_models[name] = model

        self.log(f"  [frozen-calibration] {n_frozen}/{len(models)} models "
                 f"calibrated with the training-fitted classifier frozen")
    ...
    def run_full_pipeline(self):
        ...
        if (self.config.split_validation and self.config.use_validation
            ...
            raw_models = dict(models)   # pre-calibration, for the stacking build
        ...
        # Create ensembles
        stacking_ensemble = self.create_super_ensemble(raw_models, X_train_aug, y_train_aug)
