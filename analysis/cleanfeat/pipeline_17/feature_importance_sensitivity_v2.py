...
# PATCHED COPY -- the 17 listener observables, derived from
# arm_spec.LISTENER minus the 4 that paper_names.py maps to None
# as duplicates. Not hand-written; see patch_stages.py.
# PATCHED COPY -- the 17 listener observables, derived from
# arm_spec.LISTENER minus the 4 that paper_names.py maps to None
# as duplicates. Not hand-written; see patch_stages.py.
METRICS = [
...
def load_dataset(data_root: str, n_jobs: int = -1) -> pd.DataFrame:
    # 17/8 (smoke run, STATE §25.11): under run_arm the data lives in the arm's
    # wide bundle, not in raw per-scenario CSVs — same branch cohens_d and
    # separability already had. Without it every arm run died with
    # "No CSV files found in: ./features_static".
    ...
    for cfg in ["static", "mobile"]:
        ...
        if js_path.exists():            # the shipped parameter file (dump_hp_params.py)
            ...
            raise FileNotFoundError(
                f"Missing tuned parameters: {js_path} (or {pkl_path})\n"
                f"Pass --hp-results-dir <dir with static/ and mobile/>"
            )
        # Each entry is {model_name: parameter dict} from the parameter file, or
        # {model_name: trained_estimator} from the pickle; get_params() of the latter.
        cfg_params = {}
        for name, est in best_models.items():
            try:
                cfg_params[name.lower()] = est if isinstance(est, dict) else est.get_params()
            except Exception:
...
def main():
    ...
    parser.add_argument("--k-max", type=int, default=17)
    ...
    # ── Step 4.5: derived-set member lists (cheap; ALWAYS run) ──
    # 17/8 (STATE §25.11 blocker 6): the derived sets' MEMBER NAMES were never
    # written anywhere — only counts and the recommended K — so the universal
    # set had to be extracted by hand, and k_sweep's --universal-set had no
    # artefact to consume. Placed BEFORE the --skip-evaluation return so the
    # artefact exists even on derivation-only runs (the smoke run caught the
    # first placement, inside the evaluation, being skipped).
    members_path = out_dir / "universal_set_members.csv"
    with open(members_path, "w") as f:
        f.write("method,variant,K,threshold,n_features,n_passing,members\n")
        for method in ALL_METHODS:
            all_imp_s = imp_by_method[method]["static"]
            all_imp_m = imp_by_method[method]["mobile"]
            for variant in VARIANTS:
                for K in k_values:
                    feat_list, n_pass = build_feature_set(
                        K, all_imp_s, all_imp_m, args.threshold, variant)
                    f.write(f"{method},{variant},{K},{args.threshold},"
                            f"{len(feat_list)},{n_pass},{';'.join(feat_list)}\n")
